...
from .thor import ThorInteractionCount, ThorInteractionCountComparison
...
class ThorBeaconsObjects(ThorInteractionCount):
    ...
    def step(self, action):
        ...
        # if final timestep, then compute masks for the entire trajectory and add to info
        if self.t==self.max_t:
            info['traj_masks'] = self.compute_masks(out_sz=128)
...
class ThorBeaconsFixedScaleComparison(ThorInteractionCountComparison):
    def __init__(self, config):
        super().__init__(config)
        self.history = []
        self.beacons = []
