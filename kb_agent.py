...
class KBController(object):
    ...
    def __init__(self, config):
        ...
        print("ENV_NAME", self.config.ENV_NAME)
        print("ENV_CLASS", get_env_class(self.config.ENV_NAME))

        self.envs = construct_envs(self.config, get_env_class(self.config.ENV_NAME))
        self.observation = self.envs.reset()[0]

        self.act_to_idx = collections.defaultdict(lambda: -1)
        self.act_to_idx.update(
            {act: idx for idx, act in enumerate(self.envs.call_at(0, "get_actions"))}
        )
        ...
        colors = {
            color: torch.Tensor(colors[color]).unsqueeze(1) / 255 for color in colors
        }
        self.colors = colors
        self.interactions = [
            "take",
    ...
    def render(self):
        ...
        frame = F.interpolate(
            frame.unsqueeze(0), 300, mode="bilinear", align_corners=True
        )[0]
    ...
    def step(self):
        for action in self.next_interact_command():
            if action == "interact":
                prompt = ["# Options"]
                prompt += ["Interactions: take, push, drop"]
...
if __name__ == "__main__":
    ...
    parser.add_argument("--env-name", default="ThorInteractionCountComparison-v0")
    parser.add_argument("--markers", action="store_true")
    parser.add_argument("--x_display", default=":0")
